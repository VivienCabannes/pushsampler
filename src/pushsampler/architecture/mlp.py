...
class MLP(nn.Module):
    ...
    norm: str, optional
        Specify normalization layer. Either `batch` for batch norm, `layer` for layer norm, or 'none'.
        Default is `'layer'`.
    device: str, optional
    ...
    def __init__(
        ...
        for i, out_size in enumerate(sizes[1:-1]):
            ...
            if layer_norm.lower() == "layer":
                ...
                setattr(self, "nls" + str(i), nn.Identity())
            else:
    ...
    def forward(self, x: torch.Tensor):
        for i in range(self.hidden_layers):
            # fully connected, normalizing layer and non-linearity
